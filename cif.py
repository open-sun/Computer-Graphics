...
dataloader =CIFAR10(train=True, transform=transform,download=False).set_attrs(batch_size=opt.batch_size, shuffle=True)
...
def save_image(img, path, nrow=3, padding=5):
    N,C,W,H = img.shape
    if (N%nrow!=0):
        print("N%nrow!=0")
        return
